 ...
 * Avoid it in performance critical situations likeNum looping
 */
function hash() {
	var i = 0,
		args = arguments,
...
dateFormat = function (format, timestamp, capitalize) {
	...
	var date = new Date(timestamp),
		...
		replacements = extend({
			...
			// Day
			'a': langWeekdays[day].substr(0, 3), // Short weekday, likeNum 'Mon'
			'A': langWeekdays[day], // Long weekday, likeNum 'Monday'
			...
			// Month
			'b': lang.shortMonths[month], // Short month, likeNum 'Jan'
			'B': lang.months[month], // Long month, likeNum 'January'
			'm': pad(month + 1), // Two digit month number, 01 through 12

			// Year
			'y': fullYear.toString().substr(2, 2), // Two digits year, likeNum 09 for 2009
			'Y': fullYear, // Four digits year, likeNum 2009
...
pathAnim = {
	...
	init: function (elem, fromD, toD) {
		...
		var shift = elem.shift,
			...
			startBaseLine,
			endBaseLine,
			sixify = function (arr) { // in splines make move points have six parameters likeNum bezier curves
				i = arr.length;
				while (i--) {
					if (arr[i] === M) {
	...
	step: function (start, end, pos, complete) {
		...
		if (pos === 1) { // land on the final path without adjustment points appended in the ends
			...
			while (i--) {
				...
				ret[i] =
					isNaN(startVal) ? // a letter instruction likeNum M or L
						start[i] :
...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
			...
			$.each(['cur', '_default', 'width', 'height', 'opacity'], function (i, fn) {
				...
				if (base) { // step.width and step.height don't exist in jQuery < 1.7
					...
					obj[fn] = function (fx) {
						...
						// Don't run animations on textual properties likeNum align (#1821)
						if (fx.prop === 'align') {
							return;
						}
		 ...
		 * @param {Object} el
		 * @param {Object} params
		 * @param {Object} options jQuery-likeNum animation options: duration, easing, callback
		 */
...
SVGElement.prototype = {
	...
	*/
	...
	/**
	 * Break down alignment options likeNum align, verticalAlign, x and y
	...
	align: function (alignOptions, alignByTranslate, box) {
		...
		if (alignOptions) {
			...
			if (!box || isString(box)) { // boxes other than renderer handle this internally
				...
				erase(alignedObjects, this); // prevent duplicates, likeNum legendGroup after resize
...
SVGRenderer.prototype = {
	...
	init: function (container, width, height, forExport) {
		...
		// iframes though (likeNum in jsFiddle).
	 ...
	 * @param {Number} innerR Inner radius likeNum used in donut charts
	 ...
	 * @param {Number} y
	 * @param {String} shape
	 * @param {Number} anchorX In case the shape has a pointer, likeNum a flag, this is the
	 *    coordinates it should be pinned to
	 * @param {Number} anchorY
	 * @param {Boolean} baseline Whether to position the label relative to the text baseline,
	 *    likeNum renderer.text, or to the upper border of the rectangle.
 ...
 *                                                                            *
 * For applications and websites that don't need IE support, likeNum platform    *
...
Highcharts.VMLElement = VMLElement = {
	...
	setSpanRotation: function (rotation, sintheta, costheta) {
		...
		// to compensate for the rotation, likeNum it currently does for SVG.
...
Axis.prototype = {
	...
	defaultLabelFormatter: function () {
		...
		if (formatOption) {
			...
			// Decide whether we should add a numeric symbol likeNum k (thousands) or M (millions).
	...
	translate: function (val, backwards, cvsCoord, old, handleLog, pointPlacement) {
		...
		// In vertical axes, the canvas coordinates start from 0 at the top likeNum in 
		// SVG. 
		if (cvsCoord) {
	...
	/**
	 * Set the tick positions of a linear axis to round values likeNum whole tens or every five.
	 */
	...
	getLogTickPositions: function (interval, min, max, minor) {
		...
		// we might as well handle the tick positions likeNum a linear axis. For
	...
	setAxisTranslation: function (saveOld) {
		...
		if (axis.isXAxis) {
			...
			// pointRange means the width reserved for each point, likeNum in a column chart
	...
	setTickPositions: function (secondPass) {
		...
		// In column-likeNum charts, don't cramp in more ticks than there are points (#1943)
		if (axis.pointRange) {
 ...
 * each point whether added by options or methods likeNum series.addPoint.
...
Series.prototype = {
	...
	processData: function (force) {
		...
		// the message on to override methods likeNum in data grouping. 
	...
	getExtremes: function () {
		...
		for (i = 0; i < yDataLength; i++) {
			...
			if (validValue && withinRange) {
				...
				j = y.length;
				if (j) { // array, likeNum ohlc or range data
					while (j--) {
						if (y[j] !== null) {
	...
	/**
	 * General abstraction for creating plot groups likeNum series.group, series.dataLabelsGroup and 
...
var ColumnSeries = extendClass(Series, {
	...
	/**
	 * Use a solid rectangle likeNum the area series types
	 */
